...
deploy-remote:
	ssh portfolio-domain "rm -rf ~/portfolio &> /dev/null && git clone https://github.com/leouduh/portfolio.git --depth 1  && cd ~/portfolio && git fetch && git pull && aws ecr get-login-password --region eu-west-1 | docker login --username AWS --password-stdin 214434260197.dkr.ecr.eu-west-1.amazonaws.com && docker compose pull && docker compose up -d"
